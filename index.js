...
async function tdxStart() {
    ...
    if (usePairingCode && !tdx.authState.creds.registered) {
        ...
        say("Created By シ︎𝓚𝓲𝓷𝓰👑 ༒︎𝕿𝖍𝖚𝖌𝖓𝖎𝖋𝖎𝖈𝖊𝖓𝖙༒︎💀 \nYOUTUBE : Thugnificent\nTelegram : Thugnificent\nInstagram : シ︎𝓚𝓲𝓷𝓰👑 ༒︎𝕿𝖍𝖚𝖌𝖓𝖎𝖋𝖎𝖈𝖊𝖓𝖙༒︎💀\n", {
            font: 'console',
            align: 'center',
            gradient: [randomcolor, randomcolor]
        });
        let phoneNumber = await question(`<!> Insert WhatsApp number with country code\n<✓> EXAMPLE : 2349153XXXX\n\n <+>  : `);
        let togel = phoneNumber.replace(/[^0-9]/g, '')
        await console.clear()
        let pairCode = await tdx.requestPairingCode(togel.trim());
        console.log(color(`${imageAscii}\n\n[ # ] enter that code into WhatsApp: ${pairCode}`, randomcolor));
    };
    ...
    tdx.ev.on('connection.update', async (update) => {
        ...
        try {
            ...
            if (update.connection === "open" || update.receivedPendingNotifications === "true") {
                ...
                await console.log(color("\nCreate By Thugnificent\nYOUTUBE : Thugnificent\nTelegram : Thugnificent\nInstagram : Thugnificent\n", `${randomcolor}`))
